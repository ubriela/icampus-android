...
//		ratingBar = new RatingBar(context);
	...
	private String generateEventHTML(Food food) {
		...
		if (food.getSmallphoto() != null && food.getSmallphoto().trim() != "") {	
			content += "<p><img src='" + Tools.getImageURL(food.getSmallphoto().trim()) + "' /> </br>";
		}
		if (food.getComment() != null && food.getComment().trim() != "") {
			content += "<p><b>Comment:</b> " + food.getComment().trim() + "</br>";
		}
		content += "<p><b>Rating:</b> " + food.getRating().trim() + "/5</br>";

		//		ratingBar.setNumStars(Integer.valueOf(food.getRating()));
		Log.d("FoodDialog", content);
		sb.append(content);
		return sb.toString();
	}
